...
async def create_attendance_record(payload: AttendanceRecordCreate):
    ...
    created_record = await attendance_repository.create_attendance_record(record_data)
    return created_record


@router.get("/{employee_id}")
async def get_attendance_records(employee_id: str, start_date: str, end_date: str):
    records = await attendance_repository.get_attendance_records(employee_id, start_date, end_date)
